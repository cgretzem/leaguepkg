...
Errors:
    `errors.RequestError(str)` - Cannot connect to LOL api

Misc Variables:
__version__
"""

import requests, json, random, time, urllib3, errors
...
def check_status():
    ...
    if not 'championName' in response.keys():
        return False
    else:
        return True
...
class ActiveGame:
    ...
    def updateEventList(self):
        ...
        try:
            ...
            raise errors.RequestError('Unable to retrieve Game Events')
    ...
    def loadPlayerList(self):
        try:
            ...
            raise errors.RequestError('Unable to retrieve playerlist')
        output = response.json
    ...
    def getChampName(self, player:str):
        ...
        raise errors.PlayerNotFoundException('Could not find player in playerlist')
